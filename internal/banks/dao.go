...
func (r *banksRepository) getAll() ([]Bank, error) {
	...
	if err := r.db.Select(&banks, "SELECT * FROM banks"); err != nil {
		return nil, dbErrors.DbQueryError{Err: errors.Wrap(err, dbErrors.DbQueryFail)}
	}
	return banks, nil
}
...
func (r *banksRepository) get(id int) (*Bank, error) {
	...
	if err := r.db.Get(&bank, "SELECT * FROM banks WHERE id=?", id); err != nil {
		if err == sql.ErrNoRows {
			return nil, dbErrors.EntityNotFoundError{Err: errors.Wrap(err, dbErrors.EntityNotExist)}
		}
		return nil, dbErrors.DbQueryError{Err: errors.Wrap(err, dbErrors.DbQueryFail)}
	}
	return &bank, nil
...
func (r *banksRepository) create(bank Bank) (int, error) {
	...
	if err != nil {
		return 0, dbErrors.DbQueryError{Err: errors.Wrap(err, dbErrors.DbQueryFail)}
	}
	lastID, err := result.LastInsertId()
	if err != nil {
		return 0, dbErrors.DbNotSupportedError{Err: errors.Wrap(err, dbErrors.DbNotSupported)}
	}
...
func (r *banksRepository) deleteAll() error {
	if _, err := r.db.Exec("TRUNCATE table banks"); err != nil {
		return dbErrors.DbQueryError{Err: errors.Wrap(err, dbErrors.DbQueryFail)}
	}
	return nil
}
...
func (r *banksRepository) delete(id int) error {
	...
	if affect == 0 {
		return dbErrors.EntityNotFoundError{Err: errors.Wrap(err, dbErrors.EntityNotExist)}
	}
	return nil
}
